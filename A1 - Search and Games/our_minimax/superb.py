...
class PVSAgent(MinimaxAgent):
    ...
    def minimax(self, node: Node, player: int, depth: int, alpha: Optional[float] = None,
        ...
        if node_repr is None:
            ...
            #   - also check symmetric state
            symmetric_node_repr = get_node_repr(node=node, symmetric=True)
            if symmetric_node_repr in self.EXPLORED_SET and node.depth >= self.EXPLORED_SET[symmetric_node_repr][2]:
                return self.EXPLORED_SET[symmetric_node_repr][:2]
        # 2. Get all children
